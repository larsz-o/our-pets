...
router.post('/', (req, res) => {
    ...
    const query = `INSERT INTO "pets" ("name", "species_id", "birthday", "image_path", "household_id", "medications", "feeding") VALUES ($1, $2, $3, $4, $5, $6);`; 
    for (let i = 0; i < petToAdd.pets.length; i ++) {
        let newPet = petToAdd.pets[i]; 
        console.log(newPet);
        pool.query(query, [newPet.pet_name, newPet.species_id, newPet.birthday, newPet.image_path, petToAdd.household_id, newPet.medications, newPet.feeding]).then((results) => {
            res.sendStatus(200);
        }).catch((error) => {
